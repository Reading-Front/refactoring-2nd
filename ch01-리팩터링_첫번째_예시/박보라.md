...
```ts
function amountFor (play: Play, performance: PerformanceInfo):number {
  const thisAmount= 0;
...
네이밍은 코드 리뷰를 받을 때도, 코드를 짤 떄도 항상 고민되는 영역이다.
어떻게 하면 직관적으로 다른 사람들을 이해시킬 수 있으면서도 최대한 간략하게 할 수 있을까? 라고 고민하는데, 길어져도 네이밍이 그 의미를 명확하게 전달한다면, 나 같은 경우는 생략하지 않기는 한다.

참고) 매개변수의 역할이 뚜렷하지 않은 경우 부정관사 (a/an)을 붙인다.

### 3) play 변수 제거하기

play 변수는 개별 공연 정보에서 받아괴 떄문에, 매개변수로 전달할 필요가 굳이 있을까?

<span style="color: yellowgreen; fontWegith: bold;">임시 변수를 질의 함수로 바꾸기</span>

```ts
function playFor(aPerformance: PerformanceInfo) {
...
function statement(invoice: Invoice, plays: Plays) {
  ...
  for (let performance of inovice.performances) {
    ...
    const amount = amountFor(play, performance);
